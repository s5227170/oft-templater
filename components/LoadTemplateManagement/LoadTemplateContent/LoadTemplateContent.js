    ...
    return (
        <div className={classes.LoadTemplateContent}>
            ...
            <div className={classes.Templates}>
                {props.config.loading ? <PulseLoader color={"#40cd9a"}
                    loading={props.config.loading}
                    size={30} /> : props.config.templates.map(templateName => {
                        return <label className={filename == templateName ? classes["active"] : ""} onClick={() => { filenameHandler(templateName) }}>{templateName}</label>
                    })}
            </div>
